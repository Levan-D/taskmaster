...
const Tooltip = memo(({ text, children, position, onClick, className }: Props) => {
  ...
  return (
    ...
    >
      {style && (
        <span
          className={`${positionStyle} ${className} pointer-events-none  absolute z-50  whitespace-nowrap rounded-xl border-[1px] border-neutral-200 bg-neutral-950 px-3 py-1 text-sm text-neutral-200 opacity-0   duration-200   group-hover:opacity-100`}
        >
          {text}
        </span>
      )}
